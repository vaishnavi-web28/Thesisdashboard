...
@st.cache_data
def load_and_clean():
    # ✅ CHANGE THIS TO YOUR ACTUAL RAW FILE URL
    github_csv_url = "https://github.com/vaishnavi-web28/Thesisdashboard/blob/main/software_reviews_97000.csv"
...
fig1 = px.bar(df_rating, x='rating', y='count', labels={'rating': 'Star Rating', 'count': 'Count'}, title="Number of Reviews vs Ratings")
...
matrix['sentiment_cat'] = pd.Categorical(matrix['sentiment_cat'], categories=["Very Negative", "Negative", "Neutral", "Positive", "Very Positive"], ordered=True)
matrix = matrix.sort_values(['rating', 'sentiment_cat'])

fig_treemap = px.treemap(matrix, path=[px.Constant("All Reviews"), "rating", "sentiment_cat"], values="count",
                         color="count", color_continuous_scale="Blues", title="Reviews by Star Rating and Sentiment Category")
...
fig_dh = px.imshow(heat_df, labels={'x': 'Day of Week', 'y': 'Hour of Day', 'color': 'Avg Polarity'},
                   x=heat_df.columns, y=heat_df.index, aspect='auto', color_continuous_scale='RdYlBu_r',
                   origin='lower', title="When Are Reviews Most Positive or Negative?")
...
wc = WordCloud(width=800, height=400, background_color='white', max_words=50,
               stopwords=set(nltk.corpus.stopwords.words('english'))).generate(all_text)
buf = BytesIO()
